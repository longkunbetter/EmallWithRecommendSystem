...
<html>
<head>
	<title>搜索_${requestScope.commodityData.keyword}_推一推商城</title>
		...
		<nav class="in">
			<ul class="pagination">
				...
				<c:choose>
					...
					<c:otherwise>
						<li>
							<a href="javascript:searchForPage(${requestScope.commodityData.nowPage - 1})" aria-label="Previous">
				...
				<c:forEach begin="1" end="${requestScope.commodityData.pageCount}" var="i">
					<c:choose>
						...
						<c:otherwise>
							<li>
								<a href="javascript:searchForPage(${i})">${i}<span class="sr-only"></span></a>
							</li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				...
				<c:choose>
					...
					<c:otherwise>
						<li> <a href="javascript:searchForPage(${requestScope.commodityData.nowPage + 1})" aria-label="Next"><span aria-hidden="true">»</span> </a> </li>
					</c:otherwise>
				</c:choose>
			</ul>
		</nav>
